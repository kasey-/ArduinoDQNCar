...
int main(int argc, char *argv[])
{
     ...
     * and 1 output. */
    genann *ann = genann_init(2, 2, 2, 1);

    /* Train on the four labeled data points many times. */
    double lr = 1;
    double lr_decay = 0.9999;
    for (i = 0; i < 50000; ++i) {
        genann_train(ann, input[0], &output[0], lr);
        genann_train(ann, input[1], &output[1], lr);
        genann_train(ann, input[2], &output[2], lr);
        genann_train(ann, input[3], &output[3], lr);
        lr = lr*lr_decay;
    }
    printf("%f\n",lr);
    ...
    /* save weights */
    FILE* const file = fopen("./xor.model", "w");
